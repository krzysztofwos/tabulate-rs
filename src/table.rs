...
impl Tabulator {
        ...
        Row: Serialize,
    {
        ...
        if matches!(format_name_key, Some("colon_grid")) {
            if options.col_global_align.is_none() {
    ...
    Row: Serialize,
{
    ...
    if had_object_row && columnar_keys.is_none() && !has_dataframe {
        if let Headers::Explicit(values) = &options.headers {
            if !values.is_empty() {
    ...
    if headers.len() < column_count {
        ...
        padded.extend(std::iter::repeat(String::new()).take(pad));
        padded.extend(headers.into_iter());
        headers = padded;
    }
...
fn apply_wrapping(rows: &mut [RowKind], headers: &mut [String], options: &TabulateOptions) {
    ...
    for (idx, width_opt) in header_widths.iter().copied().enumerate() {
        if let Some(width) = width_opt.filter(|w| *w > 0) {
            if max_line_width(&headers[idx], options.enable_widechars) > width {
...
impl RenderPlan {
    fn build(
        ...
        let supports_multiline = format_name_lower
            .as_deref()
            .map(|name| MULTILINE_FORMATS.iter().any(|candidate| *candidate == name))
            .unwrap_or(false);
                ...
                match marker {
                    RowMarker::Data(index) => {
                        ...
                        if let Some(next_marker) = sequence.get(idx + 1) {
                            if matches!(next_marker, RowMarker::Data(_)) {
...
fn format_float_value(value: f64, spec: &str) -> String {
    ...
    if fmt.style != FloatStyle::Fixed || !fmt.percent {
        if fmt.thousands {
...
fn pad_cell_lines(
    ...
    match alignment {
        RowAlignment::Top => {
            ...
            result.extend(std::iter::repeat(blank).take(padding));
        }
        RowAlignment::Bottom => {
            let treat_as_top = lines.len() <= 1 && format_name.starts_with("pipe");
            if treat_as_top {
                result.extend_from_slice(lines);
                result.extend(std::iter::repeat(blank.clone()).take(padding));
            } else {
                result.extend(std::iter::repeat(blank.clone()).take(padding));
                result.extend_from_slice(lines);
            }
        ...
        RowAlignment::Center => {
            ...
            result.extend(std::iter::repeat(blank.clone()).take(top));
            result.extend_from_slice(lines);
            result.extend(std::iter::repeat(blank).take(bottom));
        }
    }
    result
}
